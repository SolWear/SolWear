...
This repository is set up for AI-assisted development (Claude Code + Codex, inside Orca). The
shared engineering rules are in [`AGENTS.md`](AGENTS.md), the roles and workflow in
[`docs/AI_WORKFLOW.md`](docs/AI_WORKFLOW.md), scoped work in [`tasks/`](tasks/), and a
one-command repo runner in [`scripts/dev.sh`](scripts/dev.sh) (`setup`, `build`, `test`,
`lint`, `emulator`, `doctor`). It mirrors CI and does not replace the product docs.
